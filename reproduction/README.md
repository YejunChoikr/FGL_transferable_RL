...
September 2026 experiments. It runs independently of the original multi-machine
...
published file hashes. The numerical training algorithms are preserved from
the experiment implementation. Packaging adds the standalone runner and
...
The original release's `../models/` checkpoints use its original training and
encoding conventions. They are retained with the original code and are not
loaded by this runner. New trained checkpoints are generated by the commands
above and are excluded from Git.

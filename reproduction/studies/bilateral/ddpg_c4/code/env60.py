...
class Environment:
    ...
    def __init__(self, case, model_row, model_col, surrogate_path, y_scaler_path,
                 ...
                 row_factor=5.0, col_factor=5.0):
        self.device = device
        self.domain = domain
        # Physical half-pitch of the lattice cell. The square-cell studies used
        # (5, 5); AR0.8 has cx=8, cy=10, so the COLUMN factor becomes cx/2 = 4
        # while the row factor stays cy/2 = 5. Reusing 5.0 here would feed the
        # surrogate a location channel it was never trained on.
        self.row_factor = float(row_factor)
        self.col_factor = float(col_factor)
    ...
    def location_information(self):
        ...
        rf, cf = self.row_factor, self.col_factor
        loc_max = math.sqrt((self.row * rf) ** 2 + (self.col * cf) ** 2)
        self.location[self.x, self.y] = math.sqrt(((self.x + 1) * rf) ** 2 +
                                                  ((self.y + 1) * cf) ** 2) / loc_max
    ...
    def evaluate_action_vector(self, actions):
        ...
        This is the SECOND place the location channel is built (the first is
        ``location_information``). Both must use the instance's physical
        half-pitch factors; hard-coding 5/5 here would silently feed the
        square-cell channel to every path that skips the step loop -- the
        deterministic evaluator and the common-design surrogate audit among them.
        """
...
def make_env(task, domain, cfg, root, device):
    ...
    return Environment(
        ...
        domain=domain,
        row_factor=loc["row_factor"],
        col_factor=loc["col_factor"],
    )


def location_grid(row, col, row_factor, col_factor):
    """The (row, col) physical location channel, identical in formula to the
    frozen AR0.8 dataset's ``fea_common.location_grid``."""
    loc_max = math.sqrt((row * row_factor) ** 2 + (col * col_factor) ** 2)
    g = np.zeros((row, col), dtype=np.float64)
    for x in range(row):
        for y in range(col):
            g[x, y] = math.sqrt(((x + 1) * row_factor) ** 2 +
                                ((y + 1) * col_factor) ** 2) / loc_max
    return g


def build_state(actions, row, col, row_factor, col_factor):
    """Rebuild the 60-D surrogate input for a complete design, mirroring the
    frozen dataset's ``fea_common.build_state`` exactly (float32 query dtype)."""
    s = np.asarray(actions, dtype=np.float64).reshape(row, col)
    g = location_grid(row, col, row_factor, col_factor)
    return np.stack((s, g), axis=2).reshape(row * col, -1).flatten().astype(np.float32)

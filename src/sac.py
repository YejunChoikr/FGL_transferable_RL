"""Soft actor-critic, implemented directly in PyTorch.
  ...
  actor  : state -> 4096 -> 2048 -> 1024 -> 512 -> (mu, log_std)
  critic : (state, action) -> 4096 -> 2048 -> 1024 -> 512 -> 1, twin Q1 and Q2

Squashed-Gaussian policy with the tanh log-probability correction, twin target
critics and no target actor, and automatic entropy tuning.
"""
...
class SACAgent:
    ...
    def deterministic_action(self, states):
        """The mean action tanh(mu). Draws nothing, so evaluating the policy
        leaves the training random stream untouched."""
        s = self._to_t(states)
        with torch.no_grad():
            mu, _ = self.actor.forward(s)
        return torch.tanh(mu).cpu().numpy().reshape(-1)
